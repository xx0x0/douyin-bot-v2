...
class DouyinProcessor:
    ...
    def parse_share_url(self, share_text: str) -> dict:
        """从分享文本中提取无水印视频链接（使用移动端 API，无需登录）"""
        urls = re.findall(r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', share_text)
        if not urls:
            raise ValueError("未找到有效的分享链接")

        # 短链跳转拿 video_id
        share_response = requests.get(urls[0], headers=HEADERS, timeout=15, allow_redirects=True)
        video_id = share_response.url.split("?")[0].strip("/").split("/")[-1]
        if not video_id.isdigit():
            raise ValueError(f"无法解析视频ID: {share_response.url}")

        # 调移动端 feed API，无需 cookie
        resp = requests.get(
            MOBILE_API_URL,
        ...
        if not aweme_list:
            raise ValueError(f"移动端 API 未返回视频数据 (video_id={video_id})")
        ...
        # 图文帖子
        if data.get('images'):
            image_urls = [img['url_list'][0] for img in data['images'] if img.get('url_list')]
            return {
                "type": "images",
                "images": image_urls,
                "url": "",
    ...
    try:
        ...
        if video_info.get("type") == "images":
            return json.dumps({
                ...
                "description": f"图文标题: {video_info['title']}"
